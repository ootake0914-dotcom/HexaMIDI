/**
 * @file worker_entry.c
 * @brief SubCore 4 ワーカー ELF エントリポイント (NuttX ASMP)
 * @details mpshm 共有コンテキストにアタッチし、GM ドラム音源処理へ委譲する
 */
...
int main(void)
{
    ...
    if (!asmp_abi_ok(shared)) {
        printf("[SUB4][FATAL] ABI mismatch magic=%08x ver=%u size=%u expected %u tag=%s\n",
