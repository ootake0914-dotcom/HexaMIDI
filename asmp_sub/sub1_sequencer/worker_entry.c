/**
 * @file worker_entry.c
 * @brief SubCore 1 ワーカー ELF エントリポイント (NuttX ASMP)
 * @details mpshm 共有コンテキストにアタッチし、シーケンサー処理へ委譲する
 */
...
#define SUB_COMMON_NO_LUT /* worker_entryはLUT未使用、重複BSS削減 */
...
int main(void)
{
    ...
    if (!asmp_abi_ok(shared)) {
        printf("[SUB1][FATAL] ABI mismatch magic=%08x ver=%u size=%u expected %u tag=%s\n",
